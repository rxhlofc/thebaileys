...
const makeSocket = (config) => {
   ...
   const tryReconnect = () => {
      ...
      setTimeout(() => {
         ...
         const newSocket = (0, exports.makeSocket)(config);
         logger.info('Socket baru dibuat ulang.');
   ...
   /** send a raw buffer */
   const sendRawMessage = async (data, retries = 2) => {
      if (!ws.isOpen) {
         if (retries > 0) {
            logger.warn('Try Reconnecting..');
            tryReconnect();
            await waitForSocketOpen();
            return sendRawMessage(data, retries - 1);
         }
   ...
   const sendNode = async (frame) => {
      ...
      const bufferRaw = await sendRawMessage(buff);
      return bufferRaw
   };
   ...
   const logout = async (reason) => {
      ...
      await end(new boom_1.Boom(reason || 'Intentional Logout', {
   ...
   const waitForSocketOpen = async () => {
      ...
      await new Promise((resolve, reject) => {
         ...
         onClose = mapWebSocketError(reject);
         (async () => {
            await ws.on('open', onOpen).catch(reject);
            await ws.on('close', onClose).catch(reject);
            await ws.on('error', onClose).catch(reject);
         })();
      }).finally(async () => {
         await ws.off('open', onOpen);
         await ws.off('close', onClose);
         await ws.off('error', onClose);
      });
   };
   const startKeepAliveRequest = async () => (keepAliveReq = setInterval(async () => {
      if (!lastDateRecv) {
         lastDateRecv = new Date();
      }
      ...
      */
      if (diff > keepAliveIntervalMs + 5000) {
         await end(new boom_1.Boom('Connection was lost', {
   ...
   const sendPassiveIq = async (tag) => (query({
      tag: 'iq',
      attrs: {
         to: WABinary_1.S_WHATSAPP_NET,
         xmlns: 'passive',
   ...
   const requestPairingCode = async (phoneNumber, pairKey = "RXHLBAIL") => {
      if (pairKey) {
   ...
   ws.on('close', async () => await end(new boom_1.Boom('Connection Terminated', {
      statusCode: Types_1.DisconnectReason.connectionClosed
   })));
   // the server terminated the connection
   ws.on('CB:xmlstreamend', async () => await end(new boom_1.Boom('Connection Terminated by Server', {
      statusCode: Types_1.DisconnectReason.connectionClosed
   })));
   // QR gen
   ws.on('CB:iq,type:set,pair-device', async (stanza) => {
      ...
      const genPairQR = async () => {
         ...
         if (!refNode) {
            await end(new boom_1.Boom('QR refs attempts ended', {
         ...
         qrTimer = setTimeout(await genPairQR, qrMs);
   ...
   ws.on('CB:stream:error', async (node) => {
      logger.error({
         node
      }, 'stream errored out');
      const {
      ...
      await end(new boom_1.Boom(`Stream Errored (${reason})`, {
         statusCode,
         data: node
      }));
   });
   // stream fail, possible logout
   ws.on('CB:failure', async (node) => {
      const reason = +(node.attrs.reason || 500);
      await end(new boom_1.Boom('Connection Failure', {
         statusCode: reason,
         data: node.attrs
      }));
   });
   ws.on('CB:ib,,downgrade_webclient', async () => {
      await end(new boom_1.Boom('Multi-device beta not joined', {
